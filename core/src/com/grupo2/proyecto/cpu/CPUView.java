...
public class CPUView {
    ...
    public CPUView(final Main main, final Skin skin, AlgoCPU algoCPU) {
        ...
        if (algoCPU instanceof RoundRobin) {
            ...
            if (algoCPU.getConfig()[0] == 10) {
                vgPanel.addActor(new Label("a: " + algoCPU.getConfig()[0], skin));
            } else {
        ...
        lConmutando = new Label("conmutando", skin);
        vgPanel.addActor(lConmutando);
    ...
    public void render(float delta) {
        acumulador += delta;
        if (acumulador >= 0.8f && !termino && aux != 0) {
            acumulador -= 0.8f;
